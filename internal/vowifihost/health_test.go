...
func TestWiFiCallingHealthDoesNotTreatSMSReceiverDegradationAsCallingOutage(t *testing.T) {
	store := newWiFiCallingHealthStore()
	...
	if snapshot.State != "healthy" || snapshot.InterruptionCount != 0 {
		t.Fatalf("SMS receiver degradation interrupted calling health: %+v", snapshot)
	}
	if snapshot.InterruptedSeconds != 0 || snapshot.Availability != 100 {
		t.Fatalf("SMS receiver degradation duration = %+v", snapshot)
	}
	if len(snapshot.Events) != 1 || snapshot.Events[0].Kind != "started" {
		t.Fatalf("SMS receiver degradation events = %+v", snapshot.Events)
	}
}
...
func TestWiFiCallingHealthStartsWhenIMSIsRegistered(t *testing.T) {
	...
	if !registered.Measured || registered.State != "healthy" {
		t.Fatalf("IMS registration did not start healthy measurement: %+v", registered)
	}
	...
	if got := registered.Events[0]; got.Kind != "started" || got.State != "healthy" || got.Reason != "IMS registered" {
	...
	if snapshot.State != "healthy" || len(snapshot.Events) != 1 {
		t.Fatalf("SMS readiness changed calling health: %+v", snapshot)
	}
}
