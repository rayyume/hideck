...
test('navy-light tokens match the approved palette', () => {
  ...
  assert.match(globalStyles, /:root \{[\s\S]*--ui-radius-sm: 16px;/)
  ...
  assert.match(globalStyles, /:root \{[\s\S]*--el-border-radius-base: 16px;/)
  assert.match(globalStyles, /:root \{[\s\S]*--el-border-radius-small: 16px;/)
  assert.match(globalStyles, /:root \{[\s\S]*--el-input-border-radius: 16px;/)
...
test('primary buttons are navy pills and selected nav uses the accent bar token', () => {
  ...
  assert.match(globalStyles, /\.el-input__wrapper,\s*\.el-select__wrapper,\s*\.el-input-number,\s*\.el-date-editor \{\s*border-radius: 16px !important;/)
  assert.match(globalStyles, /\.el-menu-item \{[\s\S]*border-radius: 12px;/)
