...
test('dashboard devices phone proxy commands and tasks use defined text tokens', () => {
  ...
  const previewCard = source('../src/components/WorkspacePreviewCard.vue')
  assert.match(previewCard, /\.preview-head h2 \{[^}]*color: var\(--ui-text\)/)
...
test('login, logs, and AT terminal no longer leak the old teal palette', () => {
  ...
  assert.match(login, /background: var\(--ui-bg\)/)
  assert.match(login, /设备在线，直接进工作区/)
  assert.match(login, /弱密码登录后必须修改/)
  assert.doesNotMatch(login, /login-identity|login-access/)
