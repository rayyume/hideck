...
- 当前注册 TCP 或 port-s 监听器明确失效时，取消已不存在连接的被动观察等待，不让它阻塞实际恢复；节点真实失败的重试期限仍保留。监听器 `Accept()` 异常会报告本地错误并请求 runtime 恢复；正常关闭和退役监听器的迟到错误不影响新会话。旧 IPsec 安装的清理回调只能移除自己的策略，不能删除后来安装的新策略。这两项本地问题已用回归测试复现，不代表已证明它们就是 179 本次缺少下行的原因。
