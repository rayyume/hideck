...
- 下行验证的已尝试节点、轮次和重试期限跨 IMS/ePDG runtime 保留，重新获取相同列表或 UDP/TCP 回退不会开启新一轮。整轮结束后借用 RFC 5626 的随机指数窗口调度下一轮（这是 Vodafone 经验策略，不是把 REGISTER 200 当作注册失败）；到期才允许重新尝试旧候选，单候选也可重新获取节点/重建，不会永久停止。真实注册失败和服务端 Retry-After 仍独立约束相应节点。当前路径的迟到下行可取消等待；旧连接事件不可取消，新明确 RST/既有超时升级或当前路径 RP 回执 488 不受这轮空闲等待阻挡。普通 EOF、其他运营商不启用本策略。
