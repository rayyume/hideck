...
- 2026-09-06 的 179 日志中，port-s 在 16:22:12 超时，历史自动重连记录阻止了恢复调度，直到 16:39:14 周期注册后才重连。现将历史重连记录仅作为正常 EOF 的兼容依据（保留 2degrees 等网侧按需关闭行为）；超时、RST 和其他异常不跳过恢复，也不凭历史记录标记短信下行就绪。本地主动关闭不发起恢复。Vodafone UK/VOXI 普通异常保留 30 秒、明确 RST 保留 5 秒观察窗口；其他运营商的异常流关闭不增加这一固定等待，已有退避仍优先。普通 EOF 保留兼容观察，已观察过网侧自行重连时等待网侧；尚未观察到时维持原有 30 秒观察窗口。普通超时不触发 Vodafone 专属换节点策略。
